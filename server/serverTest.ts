...
const startServer = (port: number) => {
    ...
    wss.on("connection", (client) => {
        ...
        socketEvents.candidates = (data:any) => {
            if (ListClients[data.id]===undefined) {ListClients[data.id] = {};}
            if (ListClients[data.id].candidates==undefined) {ListClients[data.id].candidates = []}
